...
bx = float(input("beta değerini giriniz: "))
print("*" * 50, end="\n\n\n")
...
def hesap(ax, bx):
    ...
    for i in range(5, 10):
        for j in range(1, 2501):
            ...
            if studentalt <= ax / (ax + bx) <= studentust:
                z = z + 1

            if aadmalt <= ax / (ax + bx) <= aadmust:
                k = k + 1

            if madmalt <= ax / (ax + bx) <= madmust:
                m = m + 1

            if maadalt <= ax / (ax + bx) <= maadust:
                n = n + 1

            if johnsonalt <= ax / (ax + bx) <= johnsonust:
                p = p + 1

            if chenalt <= ax / (ax + bx) <= chenust:
                t = t + 1
        ...
        workbook.save(f'B({ax} {bx}).xls')  # excelisim
