...
def hesap(ax, bx):
    ...
    for i in range(5, 10):
        for j in range(1, 2501):
            x = gamma.rvs(a=ax, loc=0, scale=bx, size=i)
            ...
            if studentalt <= ax * bx <= studentust:
                z = z + 1

            if aadmalt <= ax * bx <= aadmust:
                k = k + 1

            if madmalt <= ax * bx <= madmust:
                m = m + 1

            if maadalt <= ax * bx <= maadust:
                n = n + 1

            if johnsonalt <= ax * bx <= johnsonust:
                p = p + 1

            if chenalt <= ax * bx <= chenust:
                t = t + 1
        ...
        workbook.save(f'G({ax} {bx}).xls')  # excelisim
